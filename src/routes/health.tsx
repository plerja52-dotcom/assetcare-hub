    ...
    <AppShell>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {list.map(({ i, score, band, days, next }) => {
          ...
          return (
            <Card key={i.id} className={cn("border-l-4", cardBorder[band])}>
              <CardContent className="p-5">
                <div className="flex items-start justify-between mb-3">
                  <div className="min-w-0">
                    <div className="font-semibold truncate">{i.tagNumber}</div>
                ...
                <div className={cn(
                  ...
                  overdue ? "text-primary font-medium" : dueSoon ? "text-warning-foreground dark:text-warning font-medium" : "text-muted-foreground"
                )}>
